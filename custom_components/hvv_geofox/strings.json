...
def _parse_departure(dep: dict, walk_offset_min: int) -> dict | None:
    ...
    realtime_dt = planned_dt + __import__("datetime").timedelta(minutes=delay_min)
    earliest_catch = datetime.now() + __import__("datetime").timedelta(minutes=walk_offset_min)
